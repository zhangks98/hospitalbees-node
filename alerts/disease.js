...
Request.get({url: "https://data.gov.sg/api/action/datastore_search?resource_id=ef7e44f1-9b14-4680-a60a-37d2c9dda390&sort=epi_week desc, no._of_cases desc&limit=5",
    ...
    try{
      database.query("SELECT Week as week_num, Disease_Name as Dname, No_of_cases as num_occ, ID as rank from diseases", function(err,prop){
          ...
          if(getDifference(prop[prop.length-1].week_num,week)){
            ...
            var diffArr = [0,1,2,3,4].filter(function(x){return rankArray.indexOf(x) < 0});
